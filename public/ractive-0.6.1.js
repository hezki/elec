	...
	var Binding = function( runloop, warn, create, extend, removeFromArray ) {
		...
		Binding.prototype = {
			handleChange: function() {
				var this$0 = this;
				//hezki disable toway for special elemetn
				if(this.attribute && this.attribute.node){
					if(this.attribute.node.getAttribute('toway')=='false'||this.attribute.node.getAttribute('toway')=='0'){
						runloop.start( this.root );						
						runloop.end();
						return;
					};
				};
				//hezki mark dom changes
				this.root.DOM=1;
				runloop.start( this.root );
				this.attribute.locked = true;
				this.root.viewmodel.set( this.keypath, this.getValue() );
				runloop.scheduleTask( function() {
					//hezki remove dom mark changes
					this$0.root.DOM=0;
	...
	var viewmodel$set = function( isEqual, createBranch ) {
		...
		function resolveSet( viewmodel, keypath, value ) {
			...
			valueSet = function() {
				...
				//hezki add try catch begin
				//hezki add _old
				try{
					if(!viewmodel.ractive._olds)viewmodel.ractive._olds={};//hezki
					viewmodel.ractive._olds[keypath]=parentValue[ lastKey ];//hezki
					parentValue[ lastKey ] = value;
				}catch(e){
					if(parentValue[ lastKey ] != value && viewmodel.deps && viewmodel.deps.default){//cancel by setter
						var a=viewmodel.deps.default[keypath];
						if(a.length)a.forEach(function(d){
							d.value=value;
							if(d.parentFragment&& 'value' in d.parentFragment)d.parentFragment.value=d.value;
							if(d.parentFragment && d.parentFragment.owner &&'value' in d.parentFragment.owner){
								d.parentFragment.owner.value=d.value;
								d.parentFragment.owner.locked=false;
							}
						});
						console.log('callback disallow update: '+keypath,value);
					}else throw e;
				};				
				//hezki end
			};
